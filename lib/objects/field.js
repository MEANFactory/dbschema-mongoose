...
function Field(schema, column){
  ...
  if (AUDIT_FIELDS.indexOf(this.fieldName) >= 0) {
    this.isAudit    = true;
    this.auditType  = this.isAudit ? AUDIT_TYPES[AUDIT_FIELDS.indexOf(this.fieldName)] : undefined;
  ...
  if (this.fieldName === '_id' && (['varchar', 'char', 'nvarchar', 'nchar'].indexOf(this.column.type) >= 0)) {
    this.lowercase  = true;
    this.trim       = true;
    this.type       = 'String';
  }
  if (this.column.length === 32 && this.fieldName === '_id' && this.type === 'String') {
  ...
  if (this.column.length === 36 && this.fieldName === '_id' && this.type === 'String') {
  ...
  if (this.fieldName.endsWith('_id') && this.fieldName.length > 3  && !this.column.isPrimary) {
    this.lowercase  = true;
    this.trim       = true;
    this.type       = 'String';
  }
  if (this.fieldName === 'email' && (['varchar', 'char', 'nvarchar', 'nchar'].indexOf(this.column.type) >= 0)) {
    this.lowercase  = true;
    this.trim       = true;
    this.type       = 'String';
    this.minLength  = 5;
  ...
  this.getLine = function(){
    ...
    return getPaddedFieldName(me) + ' : { ' + parts.join(', ') + ' }';
  };
...
function getMaxFieldNameLength(schema){
  var result = 0;
  schema.fields.forEach(function(field){
    if (field.fieldName && field.fieldName.length > result){
      result = field.fieldName.length;
    }
  });
  return result;
}
function getPaddedFieldName(field){
    var maxLen = getMaxFieldNameLength(field.schema);
    if (maxLen < field.fieldName.length){ maxLen = field.fieldName.length; }
